...
constexpr int kValuesPerCandidate = 4 + CLASS_NUM;
...
float probability(float value) {
    // Some exports retain the class sigmoid while others expose logits.
    return value >= 0.0f && value <= 1.0f ? value : 1.0f / (1.0f + std::exp(-value));
}

void decode(const cv::Mat& image, const float* output, std::vector<Object>* objects) {
    ...
    // The NBG tensor is planar [4 + classes, 8400], as reported by VIPLite.
    for (int candidate = 0; candidate < kCandidates; ++candidate) {
        int label = 0;
        float score = probability(output[4 * kCandidates + candidate]);
        for (int class_id = 1; class_id < CLASS_NUM; ++class_id) {
            const float candidate_score = probability(output[(4 + class_id) * kCandidates + candidate]);
        ...
        const float center_y = output[kCandidates + candidate];
        const float width = output[2 * kCandidates + candidate];
        const float height = output[3 * kCandidates + candidate];
